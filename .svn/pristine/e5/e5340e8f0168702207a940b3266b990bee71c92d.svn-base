...
namespace IGX.Geometry.Tessellation
{
    ...
    {
        private static readonly CubeTessellator cubeTessellator = new CubeTessellator();
        private static readonly EllipticalDishTessellator ellipticaldishTessllator = new EllipticalDishTessellator();
        private static readonly FacetVolumeTessellator facetvolumeTessellator = new FacetVolumeTessellator();
        private static readonly PyramidTessellator pyramidTessellator = new PyramidTessellator();
        private static readonly SphereTessellator sphereTessellator = new SphereTessellator();
        private static readonly RectangularTorusTessellator recttorusTessellator = new RectangularTorusTessellator();
        private static readonly SnoutTessellator snoutTessellator = new SnoutTessellator();
        private static readonly SphericalDishTessellator sphericaldishTessellator = new SphericalDishTessellator();
        private static readonly VolLineTessellator vollineTessellator = new VolLineTessellator();
        private static readonly CircularTorusTessllator cirdulartorusTessellator = new CircularTorusTessllator();
        private static readonly CylinderTessellator cylinderTessellator = new CylinderTessellator();

        public static readonly Dictionary<ParaPrimType, Func<PrimitiveBase, uint, bool, bool, MeshGeometry>> tessellateFunctionMap = new()
        {
            { ParaPrimType.Cube, (g, nseg, bcap, tcap) => GetTessellationResult<Cube>(g, b => cubeTessellator.Tessellate((Cube)g, nseg, bcap, tcap)) },
            { ParaPrimType.CircularTorus, (g, nseg, bcap, tcap) => GetTessellationResult<CircularTorus>(g, c => cirdulartorusTessellator.Tessellate((CircularTorus)g, nseg, bcap, tcap)) },
            { ParaPrimType.Cylinder, (g, nseg, bcap, tcap) => GetTessellationResult<Cylinder>(g, c => cylinderTessellator.Tessellate((Cylinder)g, nseg, bcap, tcap)) },
            { ParaPrimType.EllipticalDish, (g, nseg, bcap, tcap) => GetTessellationResult<EllipticalDish>(g, c => ellipticaldishTessllator.Tessellate((EllipticalDish)g, nseg, bcap, tcap)) },
            { ParaPrimType.FacetVolume, (g, nseg, bcap, tcap) => GetTessellationResult<FacetVolume>(g, c => facetvolumeTessellator.Tessellate((FacetVolume)g, nseg, bcap, tcap)) },
            { ParaPrimType.Pyramid, (g, nseg, bcap, tcap) => GetTessellationResult<Pyramid>(g, c => pyramidTessellator.Tessellate((Pyramid)g, nseg, bcap, tcap)) },
            { ParaPrimType.RectangularTorus, (g, nseg, bcap, tcap) => GetTessellationResult<RectangularTorus>(g, c => recttorusTessellator.Tessellate((RectangularTorus)g, nseg, bcap, tcap)) },
            { ParaPrimType.Snout, (g, nseg, bcap, tcap) => GetTessellationResult<Snout>(g, c => snoutTessellator.Tessellate((Snout)g, nseg, bcap, tcap)) },
            { ParaPrimType.Sphere, (g, nseg, bcap, tcap) => GetTessellationResult<Sphere>(g, c => sphereTessellator.Tessellate((Sphere)g, nseg, bcap, tcap)) },
            { ParaPrimType.SphericalDish, (g, nseg, bcap, tcap) => GetTessellationResult<SphericalDish>(g, c => sphericaldishTessellator.Tessellate((SphericalDish)g, nseg, bcap, tcap)) },
            { ParaPrimType.Line, (g, nseg, bcap, tcap) => GetTessellationResult<VolLine>(g, c => vollineTessellator.Tessellate((VolLine)g, nseg, bcap, tcap)) }
        };
        private static MeshGeometry GetTessellationResult<T>(PrimitiveBase primitive, Func<T, MeshGeometry> tessellateFunc) where T : PrimitiveBase
        {
            if (primitive is T specificPrimitive)
            {
                return tessellateFunc(specificPrimitive);
            }
            return new MeshGeometry(new List<Vector3>(), new List<Vector3>(), new List<uint>());
        }

        private readonly SemaphoreSlim semaphore = new(20); // 동시에 실행할 병렬 작업의 수를 제한 (최대 20개)
        public async Task<MeshGeometry>
            RunTessellationAsync(PrimitiveBase primitive, uint n_seg, bool bcap, bool tcap)
        {
            if (tessellateFunctionMap.TryGetValue(primitive.RenderComp.GeometryType, out Func<PrimitiveBase, uint, bool, bool, MeshGeometry>? tessellateFunc))
            {
                await semaphore.WaitAsync();
                try
                {
                    var result = tessellateFunc(primitive, n_seg, bcap, tcap);
                    return result;
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"테셀레이션 오류: {ex.Message}");
                    return new MeshGeometry(new List<Vector3>(), new List<Vector3>(), new List<uint>());// 오류 발생 시 빈/기본 튜플 반환
                }
                finally
                {
                    semaphore.Release();
            ...
            {
                Debug.WriteLine($"{primitive.RenderComp.GeometryType} 를 테셀레이션할 함수가 없음.");
                return new MeshGeometry(new List<Vector3>(), new List<Vector3>(), new List<uint>());// 함수를 찾을 수 없는 경우 빈/기본 튜플 반환
            }
        }
    }
    public class TessellationUtility
    {
        ...
        public static UnitPrimitiveMesh CreateUnitCylinder(uint n_segs, bool bCap = false, bool tCap = false)
        {
            ...
            return cachedUnitCylinders.GetOrAdd(cacheKey, key =>
            {
                ...
                SnoutTessellator temp = new SnoutTessellator();
                var mesh = temp.Tessellate(snout, n_segs, key.bCap, key.tCap);
                return new UnitPrimitiveMesh(mesh.Vertices, mesh.Normals, mesh.Indices);
            });
        }
        ...
        public static MeshGeometry SixFacetsVolume(float Xbottom = 1, float Ybottom = 1, float Xtop = 1, float Ytop = 1, float Xoffset = 0, float Yoffset = 0, float Height = 1)
        {
            ...
            return new MeshGeometry(finalPositions, finalNormals, finalIndices);
        }
        public static MeshGeometry GenerateSphereShape(uint n_longi, float radius, float arc, float sampleStartAngle, float shift_z, float scale_z, float scale, float tolerance)
        {
            ...
            return new MeshGeometry(vertices, normals, indices); // 생성된 메쉬 반환
