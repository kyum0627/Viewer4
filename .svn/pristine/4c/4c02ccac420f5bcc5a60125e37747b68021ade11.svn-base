...
namespace IGX.ViewControl.Render
{
    ...
    public class MyCamera : IMyCamera
    {
        ...
        public Matrix4 InvVP => Matrix4.Invert(ViewMatrix * ProjectionMatrix);
        ...
        public MyCamera(IMyCamera camera)
        {
            Radius = camera.Radius;
            Distance = camera.Distance;
            Pivot = camera.Target;
            SetViewportSize(camera.ViewportWidth, camera.ViewportHeight); // SetViewportSize 메서드를 통해 초기화
            ZoomScale = Radius;
            SetViewDirection(ViewDirection.LookStarboard);
        }
