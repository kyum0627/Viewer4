...
namespace IGX.Geometry.Intersect
{
    ...
    public class Intersections
    {
        ...
        /// </summary>
        /// <param name="halfspace">양분 기준선의 norID(법선, 방향), Distance(경계지점)로 구성</param>
        ...
        /// </summary>
        /// <param name="P0">line0의 Point</param>
        /// <param name="D0">line0의 Direction</param>
        /// <param name="Q0">line1의 Point</param>
        ...
        {
            return LineLine(line0.Point, line0.Direction, line1.Point, line1.Direction, out result, eps);
        }
        ...
        {
            result.type = LineLine(line.Point, line.Direction, segment.center, segment.direction, out result, eps);
        ...
        public static IntersectionType PlaneLine(Plane3f plane, Line3f line, out IntersectionResult3 result, float tolerance = MathUtil.EpsilonAngleR)
        {
            ...
            {
                if (Math.Abs(plane.distance - L0) < tolerance) // this 라인은 평면내에 존재
                {
                    result.status = IntersectionResult.INTERSECT;
                    result.type = IntersectionType.LINE;
                    result.quantity = int.MaxValue;
            ...
            else // 한 점에서 만남
            {
                ...
                result.type = IntersectionType.POINT;
                result.quantity = 1;
                result.parameter2[0] = (plane.distance - L0) / Vector3.Dot(line.direction, plane.normal);
                List<Vector3> vector3fs = new();
                result.points = vector3fs;
        ...
        public static IntersectionType PlanePlane(Plane3f plane, Plane3f other, out Line3f line, float eps = MathUtil.Epsilon)
        {
            ...
            {
                Vector3 v = (other.normal * other.distance) - (plane.normal * plane.distance);
            ...
            //두 면이 선으로 만남
            float h1 = plane.distance;
            float h2 = other.distance;
            float c1;
            float c2;
        ...
        public static IntersectionType PlaneTriangle(Plane3f plane, Triangle3f triangle, out IntersectionResult3 result)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                s[i] = Vector3.Dot(plane.normal, triangle[i]) - plane.distance; // 삼각형의 꼭지점 i 와 평면간 거리(+, -)
                if (s[i] > 0f)
                {
                    ++numPositive;
                }
...
< ((s2.X - s0.X) * (s1.Y - s0.Y));
        ...
        private static Segment3f To3D(Plane3f plane, Segment2f s, int ig)
        {
            ...
            switch (ig)
            {
                case 0:
                    y0 = s.P0.X;
                    z0 = s.P0.Y;
                    x0 = inv * (plane.distance - (plane.normal.Y * y0) - (plane.normal.Z * z0));
                    y1 = s.P1.X;
                    z1 = s.P1.Y;
                    x1 = inv * (plane.distance - (plane.normal.Y * y1) - (plane.normal.Z * z1));
                    break;
                case 1:
                    z0 = s.P0.X;
                    x0 = s.P0.Y;
                    y0 = inv * (plane.distance - (plane.normal.Z * z0) - (plane.normal.X * x0));
                    z1 = s.P1.X;
                    x1 = s.P1.Y;
                    y1 = inv * (plane.distance - (plane.normal.Z * z1) - (plane.normal.X * x1));
                    break;
                case 2:
                    x0 = s.P0.X;
                    y0 = s.P0.Y;
                    z0 = inv * (plane.distance - (plane.normal.X * x0) - (plane.normal.Y * y0));
                    x1 = s.P1.X;
                    y1 = s.P1.Y;
                    z1 = inv * (plane.distance - (plane.normal.X * x1) - (plane.normal.Y * y1));
                    break;
            }
        ...
        /// <summary>
        /// Plane3f vs Triangle의 인터섹션을 새로 정의
        /// 20240926 hschoi
        /// </summary>
        /// <param name="plane"></param>
        /// <param name="triangle"></param>
        ...
        public static IntersectionType PlaneTriangleA(Plane3f plane, Triangle3f triangle, out IntersectionResult3 result)
        {
            ...
            float constDD = plane.distance - triangle.Plane.distance;
        ...
        public static Vector3? LinePlaneIntersection(Vector3 p0, Vector3 p1, Plane3f plane)
        {
            ...
            // 교차점 t 계산
            float t = -(Vector3.Dot(plane.normal, p0) - plane.distance) / dotProduct;
