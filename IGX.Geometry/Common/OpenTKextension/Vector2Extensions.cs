...
namespace IGX.Geometry.Common
{
    public static class Vector2Extensions
    {
        ...
        public static Vector3 To3D(this Vector2 s, Plane3f plane, int coordtorecover)
        {
            ...
            switch (coordtorecover)
            {
                case 0:
                    v.Y = s.X;
                    v.Z = s.Y;
                    v.X = inv * (plane.distance - (plane.normal.Y * v.Y) - (plane.normal.Z * v.Z));
                    break;
                case 1:
                    v.Z = s.X;
                    v.X = s.Y;
                    v.Y = inv * (plane.distance - (plane.normal.Z * v.Z) - (plane.normal.X * v.X));
                    break;
                case 2:
                    v.X = s.X;
                    v.Y = s.Y;
                    v.Z = inv * (plane.distance - (plane.normal.X * v.X) - (plane.normal.Y * v.Y));
                    break;
            }
        ...
        public static Vector3 ProjectBackTo3D(this Vector2 point, Plane3f plane)
        {
            ...
            {// 평면 방정식에서 Z 값 계산
                z = -((plane.normal.X * x) + (plane.normal.Y * y) + plane.distance) / plane.normal.Z;
            }
            else
