...
namespace IGX.Geometry.DataStructure.IgxMesh
{
    public class SurfaceMesh
    {
        ...
        public int NumberOfNormals(IgxMesh mesh)
        {
        ...
        public List<uint> GetBoundaryEdgeIndices()
        {
            return BoundaryLoops
                .SelectMany(loop =>
                {
                    return Enumerable.Range(0, loop.Count)
                        .SelectMany(i =>
                        {
                            uint v0 = (uint)loop[i];
                            uint v1 = (uint)loop[(i + 1) % loop.Count];
                            return new[] { v0, v1 };
                        });
                })
                .ToList();
        }
        public List<uint> GetOutBoundaryEdgeIndices()
        {
            var loop = BoundaryLoops[0];
            return Enumerable.Range(0, loop.Count)
                .SelectMany(i =>
                {
                    uint v0 = (uint)loop[i];
        ...
        public void FindGeometryCenter(IgxMesh mesh)
        {
            ...
            foreach (int p in TriangleIndices)
            {
