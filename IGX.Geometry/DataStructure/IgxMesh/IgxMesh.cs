...
namespace IGX.Geometry.DataStructure.IgxMesh
{
    public class IgxMesh
    {
        ...
        public List<Vector3> GetOutboundaryPoints(int surfaceid)
        {
            ...
            return surf.BoundaryLoops[0]
                .Select(v => UniqueVertices[v].Position)
                .ToList();
        }
        public List<Vector3> GetBoundaryPoints(int surfaceID)
        {
            if (surfaceID < 0 || surfaceID >= Surfaces.Count)
            {
                throw new ArgumentOutOfRangeException(nameof(surfaceID), "The surface index is out of range.");
            }
            return Surfaces[surfaceID].BoundaryLoops
                .SelectMany(loop => loop
                .Select(v => UniqueVertices[v].Position))
                .ToList();
        }
        public List<uint> GetBoundaryEdgeIndices(int surfaceID)
        {
            return Surfaces[surfaceID].GetBoundaryEdgeIndices();
        }
        public List<uint> GetBoundaryEdgeIndices()
        {
            return [.. Surfaces.SelectMany(surface => surface.GetBoundaryEdgeIndices())];
        }
        public List<uint> GetOutBoundaryEdgeIndices(int surfaceID)
        {
            return Surfaces[0].GetOutBoundaryEdgeIndices();
        }
        private void MatchSurfacePairs()
        {
            ...
            for (int i = 0; i < count; i++)
                norms[i] = Surfaces[i].Normal;

            var matched = VectorPairFinder.FindOppositePairs(norms);
        }

        public void ProcessIgxMesh(ReadOnlySpan<Vertex> vtx,
        ...
        public void ProcessVertices(ReadOnlySpan<Vertex> vtx, bool mergeVertices = true)
        {
            ...
            do
            {
                ...
                for (int i = 0; i < tempSurfaces.Count; i++)
                {
                    ...
                    for (int j = i + 1; j < tempSurfaces.Count; j++)
                    {
                        ...
                        {
                            var trianglesToMerge = new List<int>(currentSurface.TriangleIndices);
                            trianglesToMerge.AddRange(tempSurfaces[j].TriangleIndices);
                            var newSurface = new SurfaceMesh(trianglesToMerge);
                            newSurface.RecalculateSurfaceMetrics(this);
                            if (newSurface.Normal.X == float.NaN)
                                break;
                            newSurface.BoundaryLoops = FindBoundaryLoops([.. GetBoundaryHalfEdgesForSurface(newSurface)], newSurface.Normal);
        ...
        {
            var boundaryHalfEdges = new List<int>();
            foreach (var triangleIndex in surface.TriangleIndices)
            {
