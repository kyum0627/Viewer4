    ...
    public class RvmLoader : ModelLoader
    {
        ...
        private void Parseb(MemoryStream stream)
        {
            ...
            Parallel.ForEach(primInsuObstChunks, s =>
            {
                ...
                using (MemoryStream localStream = new(chunkData))
                {
                    ...
                    if (volume != null)
                    {
                        ...
                        volume.RenderComp.InstanceData.GeometryID = geometryID;
                        assemblies[assyid].GeometryIDs.Add(geometryID);
            ...
            geometries = allPrimitives
                .OrderBy(item => item.RenderComp.InstanceData.GeometryID)
                .ToDictionary(item => item.RenderComp.InstanceData.GeometryID, item => item);

            //foreach (var ass in assemblies)
            //{
            //    if (ass.Value.GeometryIDs.Count > 0)
            //    {
            //        foreach (var g in ass.Value.GeometryIDs)
            //        {
            //            ass.Value.AssemblyAABB = ass.Value.AssemblyAABB.Contain(geometries[g].Aabb);
            //        }
            //        assemblies[ass.Value.ParentAssyID].AssemblyAABB = assemblies[ass.Value.ParentAssyID].AssemblyAABB.Contain(ass.Value.AssemblyAABB);
            //    }
            //}
            ...
            //{
            //    modelBoundingBox = modelBoundingBox.Contain(geometry.Aabb);
            //}
        }
        ...
        private PrimitiveBase? ReadPRIMchunkb(MemoryStream stream, int assyid, string p_i_o, uint color, bool hasTransparency = false)
        {
            ...
            if (primitive != null)
            {
                primitive.ApplyTransform(oripos);
                primitive.RenderComp.InstanceData.GeomType = primitiveTypeToParse;
                primitive.RenderComp.InstanceData.Model = oripos;
                primitive.RenderComp.GeometryType = primitiveTypeToParse;
                ...
                //modelBoundingBox = modelBoundingBox.Contain(primitive.Mesh.Aabb);
                assemblies[assyid].AssemblyAABB = assemblies[assyid].AssemblyAABB.Contain(primitive.Mesh.Aabb);
            }
            return primitive;
        }
        ...
        {
            primitive.RenderComp.GrandPrimType = p_i_o;
            primitive.RenderComp.InstanceData.GeometryID = geometries.Count;
            primitive.RenderComp.InstanceData.EassemblyID = parent;
