    ...
    public class RevLoader : ModelLoader
    {
                    ...
                    case "INSU":
                        ...
                        if (volume != null)
                        {
                            currentAssy.AddGeometry(volume);//.GeometryID);
                                                            //(Volume.MeshGeometry.Vertices, Volume.MeshGeometry.Normals, Volume.MeshGeometry.Indices, Volume.MeshGeometry.edge_inds) =
                            TessellationManager.tessellateFunctionMap[volume.RenderComp.InstanceData.GeomType].Invoke(volume, TessellationUtility.default_nsegs, true, true);
                            geometries[volume.RenderComp.InstanceData.GeometryID] = volume;
                            //currentAssy.AssemblyAABB = currentAssy.AssemblyAABB.Contain(volume.Aabb);// InstanceData.Aabb);
                            //if (!(currentAssy.ParentAssyID == -1 || !assemblies.ContainsKey(currentAssy.ParentAssyID))) // ParentID가 유효한 경우에만 접근
                            //{
                            //    assemblies[currentAssy.ParentAssyID].AssemblyAABB = assemblies[currentAssy.ParentAssyID].AssemblyAABB.Contain(volume.Aabb);
                            //}
                            //modelBoundingBox = modelBoundingBox.Contain(volume.Aabb);
                        }
                        break;
                    case "CNTE":
                        if (!(currentAssy.ParentAssyID == -1 || !assemblies.ContainsKey(currentAssy.ParentAssyID))) // ParentID가 유효한 경우에만 접근
                        {
                            //assemblies[currentAssy.ParentAssyID].AssemblyAABB = assemblies[currentAssy.ParentAssyID].AssemblyAABB.Contain(currentAssy.AssemblyAABB);
                            assemblies[currentAssy.ParentAssyID].SubEbom.Add(currentAssy.ID);
                        }
        ...
        PrimitiveBase? ReadPRIMChunk(int parent, uint color, string prim_obst_insu, bool hasTransparency = false)
        {
            ...
            if (primitive != null)
            {
                ...
                primitive.RenderComp.InstanceData.Model = oripos;
                primitive.RenderComp.InstanceData.GeomType = prmtyp;
                primitive.RenderComp.InstanceData.Model = oripos;
                primitive.RenderComp.GeometryType = prmtyp;
                if (primitive.RenderComp.GeometryType != ParaPrimType.FacetVolume)
                {
                    (primitive.Mesh.Aabb, primitive.Mesh.Oobb) = GeometryFactory.UpdateAabbOobb(oripos, aabb);
                }
            }
            else
            {
            ...
            {
                primitive.RenderComp.GrandPrimType = prim_obst_insu;
                primitive.RenderComp.InstanceData.EassemblyID = parent;  // 현재 부모 MeshID 설정
                primitive.RenderComp.ColorID = color;
                primitive.RenderComp.InstanceData.GeometryID = geometries.Count;
                return primitive;
            }
            // --------
        ...
        private static Cube? ReadBoxRev(Matrix4 matrix, AABB3 bBoxLocal)
        { // x, y, z
