...
namespace IGX.Loader
{
    ...
    public class Model3D : IModel3D, IEquatable<Model3D>
    {
        ...
        public void AddGeometry(PrimitiveBase p, int? assyID = null)
        {
            // 시스템에서 새롭게 생성된 프리미티브의 추가 혹은 삭제....
            if(Geometries.TryGetValue(p.GeometryID, out var g))
            {//데이터 중복
                return;
            }
            Geometries[p.GeometryID] = p;
            if(assyID == null)
            {
            ...
            {
                if(Eassemblies.TryGetValue((int)assyID, out var ass))
                {
                    //...
                }
            }
        ...
        public Model3D SetGeometrySelectionMode(int geometryId, SelectTo sm = SelectTo.None)
        {
            ...
            {
                primitive.RenderComp.InstanceData.SelectionMode = sm;
            }
            return this;
        }
        ...
        public Model3D SetGeometryLayer(int geometryId, int layer)
        {
            if (Geometries.TryGetValue(geometryId, out var primitive))
            {
                primitive.RenderComp.InstanceData.Layer = layer;
            }
            return this;
        }
        ...
        public Model3D SetGeometryColor(int geometryId, Vector4 color, bool transparent)
        {
            if (Geometries.TryGetValue(geometryId, out var primitive))
            {
                primitive.RenderComp.InstanceData.Color = color;
            }
            return this;
        }
        ...
        public List<int> GetGeometryIdsOfThePart(int partId) => [.. Geometries.Where(part => part.Value.RenderComp.InstanceData.EassemblyID == partId).Select(part => part.Key)];
        public Vector4 GetGeometryColor(int geometryId) => Geometries.TryGetValue(geometryId, out var primitive) ? primitive.RenderComp.InstanceData.Color : default;
        public int GetGeometryLayer(int geometryId) => Geometries.TryGetValue(geometryId, out var primitive) ? primitive.RenderComp.InstanceData.Layer : -1;
        public GeometryInstance? GetGeometryInstance(int geometryId) => Geometries.TryGetValue(geometryId, out var primitive) ? primitive.RenderComp.InstanceData : null;
        ...
        public int GetAssemblyIndex(int geometryId) => Geometries.TryGetValue(geometryId, out var primitive) ? primitive.RenderComp.InstanceData.EassemblyID : -1;
        public ParaPrimType GetPrimitiveType(int geometryId) => Geometries.TryGetValue(geometryId, out var primitive) ? primitive.RenderComp.GeometryType : default;
        public string? GetGrandPrimitiveType(int geometryId) => Geometries.TryGetValue(geometryId, out var primitive) ? primitive.RenderComp.GrandPrimType : null;
